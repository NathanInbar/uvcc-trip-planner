<script>
    ...
    function plan_trip()
    {
        ...
        function rotate_arb_trip(){
            ...
            do{
                if (arb_trip_i == Object.keys(trips).length)
                    arb_trip_i = 0;
                else
                    arb_trip_i +=1;
            }while(Object.keys(trips)[arb_trip_i].is_full_max(max_cavers))
        ...
        let i = 0;
        while(count_full_trips_seats()<Object.keys(trips).length && i < $cavers.length)//while we have open seats
        {
            let caver = $cavers[i];
